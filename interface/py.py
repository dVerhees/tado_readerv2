...
class Tado:
    ...
    def getPower(self, zone):
        """Gets Current boiler power """
        data = self.getState(zone)['activityDataPoints']
        return { 'power' : data['heatingPower']['percentage']}
    ...
    def getOutsideTemp(self):
        """Gets outside temperature and solar intensity"""
        data = self.getWeather()
        return { 'outsideTemperature' : data['outsideTemperature']['celsius'],
                 'solarIntensity' : data['solarIntensity']['percentage']}
